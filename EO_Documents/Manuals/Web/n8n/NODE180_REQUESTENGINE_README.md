...
| ノード名 | 設定内容 |
|---------|---------|
| #280AWS-Singapore | Lambda関数: `tnp-re-d1-lambda-apse1` |
| #280AZ-HongKong | Azure Functions URL: `https://tnp-re-d1-funcapp-eastasia.azurewebsites.net/...` |
...
| Function | Lambda関数名（例: `tnp-re-d1-lambda-apse1`） |
| Payload | `={{ $json.data }}` |

4. **接続**: 出力を `295 JSON result REMOVER` または `340 Random Sleep (ms)` に接続

```
AWS Lambda関数名の命名規則: tnp-re-{environment}-lambda-{area}
例: tnp-re-d1-lambda-apne1 (東京)
    tnp-re-d1-lambda-apse1 (シンガポール)
```
