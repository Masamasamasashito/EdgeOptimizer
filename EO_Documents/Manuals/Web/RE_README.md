...
- `280AZ-japan-east RequestEngine KeyVault`ノード
    ...
    - Azure > 関数アプリ > 関数 > リソースJSON > invoke_url_template
     `https://tnp-re-d1-funcapp-jpe-xxxxxxxxxxxxxxxx.japaneast-01.azurewebsites.net/api/requestengine_func`
...
個人GCPメール > IAMと管理 > サービスアカウント > `tnp-gsa-d1-oa2inv-asne1`を選択 > 鍵
...
- **リクエストエンジン名**: リソースグループ: `tnp-re-d1-resource-grp-jpe`、関数アプリ: `tnp-re-d1-funcapp-jpe`、関数名: `requestengine_func`
- **Key Vault URI**: `https://tnp-re-d1-kv-jpe.vault.azure.net/` (GitHubシークレット: `EO_AZ_RE_KEYVAULT_URL`)
- **シークレット識別子**: `https://tnp-re-d1-kv-jpe.vault.azure.net/secrets/AZFUNC-REQUEST-SECRET/{VERSION}`
- **Key Vault**: `tnp-re-d1-kv-jpe`
...
- **リクエストエンジン名**: 組織: `<組織ドメイン>`、プロジェクト名: `tnp-re-d1-pr-asne1`、Cloud Runサービス: `tnp-re-d1-cloudrun-asne1`
- **リソース名**: `projects/tnp-re-d1-pr-asne1/secrets/tnp-re-d1-secretmng/versions/{VERSION}`
- **シークレット**: `tnp-re-d1-secretmng`
...
- **リクエストエンジン名**: Workersアプリケーション: `tnp-re-d1-cfworker-global`、カスタムドメイン: `tnp-re-d1-cfworker-global.sample.com`
