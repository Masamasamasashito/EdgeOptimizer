...
|---|---|---|---|
| AWS | `RequestEngine/Web/aws/lambda/py/` | CloudFormation (`CFn/`) | `lambda{NNN}.env` |
| Azure | `RequestEngine/Web/azure/functions/py/` | Bicep (`bicep/`) | `funcapp{NNN}.env` |
| GCP | `RequestEngine/Web/gcp/cloudrun/py/` | Terraform (`terraform/`) | `cloudrun{NNN}.env` |
| CF | `RequestEngine/Web/cf/workers/ts/` | Wrangler (`funcfiles/wrangler.toml`) | `cfworker{NNN}.env` |

### 2.3 クラウド4種の実際の .env 定義

**AWS Lambda** (`RequestEngine/Web/aws/lambda/py/instances_conf/lambda001.env`)

```env
# Referenced by (この定義値を使用するファイル):
#   ../../../../../.github/workflows/deploy-py-to-aws-lambda-web.yml
#   ../CFn/re-aws-cfnstack.yml (パラメータ化済み)
EoServiceServerless=lambda
EoNanoidSlug=001
EoArea=ap-northeast-1
EoAreaShort=apne1
```

**Azure Functions** (`RequestEngine/Web/azure/functions/py/instances_conf/funcapp001.env`)
```env
# Referenced by (この定義値を使用するファイル):
#   .github/workflows/deploy-py-to-az-function-web.yml (FUNCTION_APP_NAME, RESOURCE_GROUP)
#   RequestEngine/Web/azure/functions/py/bicep/tnp-re-d1-azure-funcapp.bicep (パラメータ化済み)
...
**GCP Cloud Run** (`RequestEngine/Web/gcp/cloudrun/py/instances_conf/cloudrun001.env`)
```env
# Referenced by (この定義値を使用するファイル):
#   .github/workflows/deploy-py-to-gcp-cloudrun-web.yml (EO_GCP_CLOUD_RUN_SERVICE_NAME, EO_GCP_REGION)
#   RequestEngine/Web/gcp/cloudrun/py/terraform/cloud_run.tf (サービス名構築に使用)
#   RequestEngine/Web/gcp/cloudrun/py/terraform/main.tf (name_prefix + EO_REGION_SHORT)
...
**Cloudflare Workers** (`RequestEngine/Web/cf/workers/ts/instances_conf/cfworker001.env`)
```env
# Referenced by (この定義値を使用するファイル):
#   .github/workflows/deploy-ts-to-cf-worker-web.yml (wrangler.toml name)
...
`EO_RE_INSTANCE_ID` は `RequestEngine/Web/aws/lambda/py/instances_conf/*.env` や `RequestEngine/Web/azure/functions/py/instances_conf/*.env` など、各クラウドの `RequestEngine/*/*/*/instances_conf/*.env` に存在する。基本的にクラウドとリージョンの中でRequestEngine自体に付与するインスタンスIDであり、リソース名の最後尾に付与することで、リソースのグローバルなユニーク識別子とする。
...
実名例）AzureのFunction App`tnp-re-d1-funcapp-jpe-a1b2-001`
...
IaC: `RequestEngine/Web/aws/lambda/py/CFn/re-aws-cfnstack.yml`

| リソース | パターン | 例 | 文字制限 |
|---|---|---|---|
| **Lambda 関数** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}` | `tnp-re-d1-lambda-apne1` | 64文字 |
| **Lambda Log Group** | `/aws/lambda/{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}` | `/aws/lambda/tnp-re-d1-lambda-apne1` | — |
| **Lambda Layer** | `{ProjectPrefix}-{Component}-{Environment}-lambda-python-slim-layer` | `tnp-re-d1-lambda-python-slim-layer` | 自動採番あり |
| **Secrets Manager** | `{ProjectPrefix}-{Component}-{Environment}-secretsmng-{RegionShort}` | `tnp-re-d1-secretsmng-apne1` | 512文字 |
| **IAM Role (Lambda実行)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-role` | `tnp-re-d1-lambda-apne1-role` | 64文字 |
| **IAM Policy (基本実行)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-basic-exec-iamp` | `tnp-re-d1-lambda-apne1-basic-exec-iamp` | 128文字 |
| **IAM Policy (SM アクセス)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-role-iamp` | `tnp-re-d1-lambda-apne1-role-iamp` | 128文字 |
| **IAM User (n8n呼び出し)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-iamu` | `tnp-re-d1-lambda-apne1-iamu` | 64文字 |
| **IAM Policy (n8n invoke)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-access-key-iamp` | `tnp-re-d1-lambda-apne1-access-key-iamp` | 128文字 |
| **IAM Role (GH Actions)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-ghactions-deploy-iamr` | `tnp-re-d1-lambda-apne1-ghactions-deploy-iamr` | 64文字 |
| **IAM Policy (GH Actions)** | `{ProjectPrefix}-{Component}-{Environment}-lambda-{RegionShort}-ghactions-deploy-iamr-iamp` | `tnp-re-d1-lambda-apne1-ghactions-deploy-iamr-iamp` | 128文字 |
...
IaC: `RequestEngine/Web/azure/functions/py/bicep/tnp-re-d1-azure-funcapp.bicep`
...
|---|---|---|---|
| **Function App** | `{EO_PROJECT}-{EO_COMPONENT}-{EO_ENV}-funcapp-{EO_REGION_SHORT}-{EO_GLOBAL_PRJ_ENV_ID}-{EO_RE_INSTANCE_ID}` | `tnp-re-d1-funcapp-jpe-a1b2-001` | 60文字 |
...
| **Resource Group** | `{EO_PROJECT}-{EO_COMPONENT}-{EO_ENV}-resourcegrp-{EO_REGION_SHORT}` | `tnp-re-d1-resourcegrp-jpe` | 90文字 |
...
IaC: `RequestEngine/Web/gcp/cloudrun/py/terraform/`

| リソース | パターン（Terraform） | 例 | 文字制限 |
|---|---|---|---|
| **Cloud Run Service** | `{name_prefix}-cloudrun-{region_short}` | `tnp-re-d1-cloudrun-asne1` | 63文字 |
| **Artifact Registry** | `{name_prefix}-ar-{region_short}` ※ | `tnp-re-d1-ar-asne1` | 63文字 |
| **Secret Manager** | `{name_prefix}-secretmng` | `tnp-re-d1-secretmng` | 255文字 |
...
IaC: `RequestEngine/Web/cf/workers/ts/funcfiles/wrangler.toml`（または `wrangler.jsonc`）

**§2.4 に従い** Worker 名は一意スコープがアカウント内で名前で識別するため、`EO_RE_INSTANCE_ID` を含める。

| リソース | パターン | 例 |
|---|---|---|
| **Worker 名** | `{EO_PROJECT}-{EO_COMPONENT}-{EO_ENV}-cfworker-{EO_REGION_SHORT}-{EO_RE_INSTANCE_ID}` | `tnp-re-d1-cfworker-global-001` |
...
| **AWS IAM Role** | 64文字 | `tnp-re-d1-lambda-apne1-ghactions-deploy-iamr` | 43文字 | 21文字 |
| **AWS IAM Policy** | 128文字 | `tnp-re-d1-lambda-apne1-ghactions-deploy-iamr-iamp` | 48文字 | 80文字 |
| **Azure Function App** | 60文字 | `tnp-re-d1-funcapp-jpe-a1b2-001` | 29文字 | 31文字 |
| **Cloud Run Service** | 63文字 | `tnp-re-d1-cloudrun-asne1` | 23文字 | 40文字 |
...
| パターン | 例 | 文字数 |
|---|---|---|
| `{EO_PROJECT}-{EO_COMPONENT}-{EO_ENV}-pr-{EO_REGION_SHORT}` | `tnp-re-d1-pr-asne1` | 18文字 ✅ |
