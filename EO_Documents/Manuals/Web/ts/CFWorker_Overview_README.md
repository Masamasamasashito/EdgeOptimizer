...
esbuild（`RequestEngine/Web/cf/workers/ts/funcfiles/build.mjs`）が `bundle: true` で全 `import` を解決し、`dist/worker.js` に1ファイルにバンドル。
...
### 1. Prerequisites
Ensure `.github/workflows/deploy-ts-to-cf-worker-web.yml` exists.
