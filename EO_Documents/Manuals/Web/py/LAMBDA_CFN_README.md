...
`RequestEngine\Web\aws\lambda\py\CFn\re-aws-cfnstack.yml` を使用した AWS Lambda Request Engine 構築手順です。
...
>
> 📦 [`RequestEngine/Web/aws/lambda/py/funcfiles/requests-py314-slim-layer.zip`](../../../RequestEngine/Web/aws/lambda/py/funcfiles/requests-py314-slim-layer.zip)
>
...
# CFn実行時に必要なため、メモしておく
# バージョン固定は`EO_RequestEngine/Web/aws/lambda/py/localdev/env.example`を複製した`.env`の`COMMON_LAMBDA_LAYER_DOCKER_IMAGE_TAG`で`python3.14-slim`のように指定可能
...
作成された zip ファイル: `EO_RequestEngine/Web/aws/lambda/py/funcfiles/requests-python-slim-layer.zip`
...
4. 「テンプレートファイルのアップロード」で `RequestEngine/Web/aws/lambda/py/CFn/re-aws-cfnstack.yml` を選択
5. 次へ
...
aws cloudformation create-stack \
  ...
  --template-body file://RequestEngine/Web/aws/lambda/py/CFn/re-aws-cfnstack.yml \
  --parameters \
...
- コマンド実行時は、`--template-body file://RequestEngine/Web/aws/lambda/py/CFn/re-aws-cfnstack.yml` のパスがテンプレートの実際の場所を指すようにしてください（上記はリポジトリルートで実行する場合の相対パスです）。
...
### 5-1. GitHub Actions シークレット (Secrets) の設定
...
ワークフロー `.github/workflows/deploy-py-to-aws-lambda-web.yml` は、Lambda 関数名・リージョン・ランタイムなどを **GitHub リポジトリの Variables** から読みます。`re-aws-cfnstack.yml` でデプロイしたスタックのパラメータと一致する値を設定してください。
...
**注意:** GitHub では Variables の「名前」が登録時に自動で**すべて大文字**に変換されます。UI で `EoProject` と入力しても `EOPROJECT` として保存されるため、ワークフロー側では大文字で参照しています（CFn のパラメータ名 EoProject 等とは別の表記です）。

| Variable name（GitHub に登録後は大文字で表示） | 推奨値（CFn デフォルトと同一） | 説明 |
|----------------------------------------------|-------------------------------|------|
| `EOPROJECT` | `re` | プロジェクト識別子（Request Engine） |
| `EOENVIRONMENT` | `d1` | 環境識別子（dev01 等） |
| `EOSERVICESERVERLESS` | `lambda` | サーバレス識別子（AWS Lambda） |
...
4. 各変数の **Value** を入力 > **Add variable** で保存（名前は大文字・小文字どちらで入力しても GitHub が大文字に変換します）
...
- リポジトリに `.github/workflows/deploy-py-to-aws-lambda-web.yml` が存在することを確認してください。
