...
**対象ファイル:** `RequestEngine/Web/aws/lambda/py/CFn/re-aws-cfnstack.yml`
...
- `RequestEngine\Web\aws\lambda\py\CFn\re-aws-cfnstack.yml`にて。

8. **各種.bicep を プレフィックス無しにリネーム**
   - `EO_Documents\Manuals\SchemaDesign_DbNormalization.md`にて。
     - RequestEngine\Web\azure\functions\py\bicep\eo-re-d1-azure-funcapp.bicep　を RequestEngine\Web\azure\functions\py\bicep\az-funcapp.bicepにリネーム
     - RequestEngine\Web\azure\functions\py\bicep\eo-re-d1-az-child-mgmt-grp.bicep　を RequestEngine\Web\azure\functions\py\bicep\az-child-mgmt-grp.bicepにリネーム
     - RequestEngine\Web\azure\functions\py\bicep\eo-re-d1-az-child-mgmt-grp-policies.bicep　を RequestEngine\Web\azure\functions\py\bicep\az-child-mgmt-grp-policies.bicepにリネーム
...
2. **ワークフロー（deploy-py-to-aws-lambda-web.yml）の `function-name`**
...
**対象ファイル:** `.github/workflows/deploy-py-to-aws-lambda-web.yml`
...
**対象ファイル:**
- `RequestEngine/Web/aws/lambda/py/instances_conf/lambda001.env`
- `RequestEngine/Web/aws/lambda/py/instances_conf/env.example`
...
- [ ] `.github/workflows/deploy-py-to-aws-lambda-web.yml` の `LAMBDA_FUNCTION_NAME` がテナント名付きの想定値になっている（リソース名を変える場合は別手順）
...
- [ ] LAMBDA_README.md: パラメータ名の言及を新名に。リソース名例をテナント名（または <テナント名>）に。ワークフロー名は `deploy-py-to-aws-lambda-web.yml`
...
| ファイル | 変更内容 |
|----------|----------|
| `RequestEngine/Web/aws/lambda/py/CFn/re-aws-cfnstack.yml` | パラメータ名 6 個のリネーム、全参照・Metadata・Tags の Key を新名に統一 |
| `.github/workflows/deploy-py-to-aws-lambda-web.yml` | コメントのパターン表記を新命名に合わせる（env の値はそのまま） |
...
| `RequestEngine/Web/aws/lambda/py/instances_conf/lambda001.env` | 変更不要（必要ならコメントのみ） |
| `RequestEngine/Web/aws/lambda/py/instances_conf/env.example` | 同上 |
