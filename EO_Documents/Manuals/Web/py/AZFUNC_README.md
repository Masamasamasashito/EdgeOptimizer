...
`RequestEngine/Web/azure/functions/py/localdev/` は Azure Functions ローカル開発用のDocker環境です。
...
- **本番デプロイ**: GitHub Actions（`.github/workflows/deploy-py-to-az-function-web.yml`）で実行。この Docker 環境はローカル開発のみに使用

```
RequestEngine/Web/azure/functions/py/
├── localdev/
...
- `cd RequestEngine\Web\azure\functions\py\localdev`
- `mkdir ../funcfiles`
- `cp env.example .env`
- .env を編集する
...
1. `cd RequestEngine\Web\azure\functions\py\localdev`
2. コンテナ中に入る
   - `docker compose run --rm azfunc_builder bash`
3. 開発作業
...
1. `RequestEngine/Web/azure/functions/py/funcfiles/function_app.py`、`requirements.txt`、または`host.json`を変更
2. `main`ブランチにプッシュ
...
1. `RequestEngine/Web/azure/functions/py/funcfiles/` ディレクトリ内のファイルを変更
2. `main`ブランチにプッシュ
...
```bash
cd RequestEngine/Web/azure/functions/py/funcfiles
