...
`re-aws-cfnstack.yml` を使用した AWS Lambda Request Engine インフラストラクチャの構築手順です。
...
デフォルトパラメータ（`tnp-re-d1-lambda-apne1`）の場合：

| リソース種別 | リソース名 |
|-------------|-----------|
| Lambda 関数 | `tnp-re-d1-lambda-apne1` |
| Lambda 実行ロール | `tnp-re-d1-lambda-apne1-role` |
| Lambda 基本実行ポリシー | `tnp-re-d1-lambda-apne1-basic-exec-iamp` |
| Lambda シークレットアクセスポリシー | `tnp-re-d1-lambda-apne1-role-iamp` |
| Secrets Manager | `tnp-re-d1-secretsmng-apne1` |
| CloudWatch Logs | `/aws/lambda/tnp-re-d1-lambda-apne1` |
| n8n用 IAM ユーザー | `tnp-re-d1-lambda-apne1-iamu` |
| n8n用 IAM ポリシー | `tnp-re-d1-lambda-apne1-access-key-iamp` |
| GitHub OIDC プロバイダー | `token.actions.githubusercontent.com` |
| GitHub Actions デプロイロール | `tnp-re-d1-lambda-apne1-ghactions-deploy-iamr` |
| GitHub Actions デプロイポリシー | `tnp-re-d1-lambda-apne1-ghactions-deploy-iamr-iamp` |
...
**既存で存在すしない場合の対応:**
STEP 2 で`re-aws-cfnstack.yml` を実行します。
---
...
5. 次へ
6. スタック名: `tnp-re-d1-lambda-apne1-stack-yyyymmdd-001`（任意）
7. パラメータを入力:
...
| EoReLambdaLayerName | `re-d1-lambda-python-slim-layer` | STEP 1-1 で作成した Layer 名 |
...
aws cloudformation create-stack \
  --stack-name tnp-re-d1-lambda-apne1-stack-yyyymmdd-001 \
  --template-body file://re-aws-cfnstack.yml \
  --parameters \
    ...
    ParameterKey=EoReLambdaLayerName,ParameterValue=re-d1-lambda-python-slim-layer \
...
1. AWS コンソール > Secrets Manager > `tnp-re-d1-secretsmng-apne1`
...
aws secretsmanager put-secret-value \
  --secret-id tnp-re-d1-secretsmng-apne1 \
...
1. AWS コンソール > IAM > ユーザー > `tnp-re-d1-lambda-apne1-iamu`
2. 「セキュリティ認証情報」タブ
3. 「アクセスキーを作成」
4. 「AWS の外部で実行されるアプリケーション」を選択
5. 説明タグ: `tnp-re-d1-lambda-apne1-iamu-access-key`
6. 「アクセスキーを作成」
7. **CSV をダウンロード**（`tnp-re-d1-lambda-apne1-iamu_accessKeys.csv`）
...
3. Function Name or ID: `tnp-re-d1-lambda-apne1`
4. 「Save」
...
- 例: `arn:aws:iam::<AWSアカウントID>:role/tnp-re-d1-lambda-apne1-ghactions-deploy-iamr`
...
- **LambdaにWarmupリクエストを送るn8nノード設定**:
    ...
    - Function Name or ID: `tnp-re-d1-lambda-apne1`を入力
...
| EoReLambdaLayerName | `re-d1-lambda-python-slim-layer` | Lambda Layer 名 |
