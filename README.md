...
|-----|------------------|----------------|
| **アセットWarmup** | ❌ メインドキュメント止まり | ✅ メインドキュメント(HTML)/CSS/JS/画像/フォント全対応 |
| **GEO分散リクエスト** | ❌ 単一ロケーション | ✅ AWS/Azure/GCP/CF Workers |
| **現地CDNエッジWarmup** | ❌ ツール実行地のみ | ✅ ユーザー現地エッジを直接Warmup |
| **多層CDN対応** | ❌ 不可 | ✅ 可能 |
| **XMLサイトマップURLフィルタリング** | ❌ 無し | ✅ キャッシュすべきURLのみにフィルタリング可能 |
| **バリアント対応** | ❌ 固定UA or 無配慮 | ✅ User-Agent/Accept-Languageなど自由設定可能 |
| **セキュリティ** | ⚠️ 簡易的 | ✅ 4層（DNS認証/クラウドIAM認証/トークン照合/レート制御） |
| **オーケストレーション** | ❌ なし | ✅ n8n自動化 |
...
| Web Analytics | GA4, Adobe Analytics | ⚠️一部（Core Web Vitals等） | ✅ | ❌ |
| **Edge Optimizer / Analyzer** | **にしラボOSS** | **✅** | **✅（機会損失$）** | **✅** |

**→ 「遅延 = お金」という橋渡しをし、さらに自動最適化するツールは Edge Analyzer しかない**

エンジニアが「この改善でどれだけ売上インパクトがあるか」を、ダッシュボードで直接確認できることを目指しています。  
また、パフォーマンス改善タスクを、CVR・売上インパクト順に優先度付けできるのが Edge Analyzer のゴールです。
...
| さくらウェブアクセラレータ | `x-webaccel-origin-status` |
| Bunny CDN | `cdn-pullzone` |
| Alibaba Cloud CDN | `eagleid` |
| CDNetworks | `x-cnc-request-id` |
| KeyCDN | `x-pull` |
| GCP Cloud CDN / Media CDN | `server`（google-edge-cache）、`cdn_cache_status` |

## コスト

- **維持費**: ゼロ円（OSS/ローカルでセルフホスティング可能）
- **稼働コスト**: サーバーレス関数と各種クラウドのシークレットサービス + ローカルでセルフホスティングしたn8n実行環境
...
| ユースケース | 説明 |
|-------------|------|
| **Edge AI 時代のエッジWarmup・計測** | Edge AI 推論・API を世界中から Warmup しレイテンシを可視化 |
