...
EO_Architecture/
  ...
  llm/local/Architecture.md
  llm/cloud/Architecture.md
...
RequestEngine/                  # 結論: 下記ツリーへ移行（現状のルート直下 aws/ 等は RequestEngine/Web/ へ移す想定）
  ...
  llm/
    local/                      # ローカル LLM 向け
    cloud/                      # GEO 分散・クラウド LLM プローブ向け（開発は複製後に本格化）
```
...
- `RepoRestructure/Add_LLM_dir/` … 本計画メモ（本ファイル）
