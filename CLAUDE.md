...
- `MCNE_Documents/` - MultiCloudNamingEngine（MCNE）のドキュメント・参照。マルチクラウドリソース命名を一元化する専用ライブラリ開発用。
- `test/` - **.gitignore** に含まれるファイルの置き場。このディレクトリ内のファイルは他のディレクトリへの移動や複製は禁止。
...
```json
{
  "data": {
    ...
    "headersForTargetUrl": {"User-Agent": "...", "Accept-Language": "..."},
    "httpRequestNumber": "...",
    "httpRequestUUID": "...",
    "httpRequestRoundID": "..."
  }
...
- **Naming convention**: Resources follow `nis-re-d1-{service}-{region}` pattern
